...
Coordinator chỉ ghi output khi Verifier trả danh sách lỗi rỗng. Một batch mới gọi `start_trace()` để thay thế trace run cũ, không append history cũ.
...
| Order & Product Agent | M2 | orders, items, products, sellers, category translation | handoff | Entity chính, product/category và raw evidence. |
...
Mọi handoff dùng envelope trong `src/contracts.Handoff`:
...
`order`, `items` và `payments` là raw row CSV đã parse; danh sách giữ thứ tự nguồn. `products` có `product_id` và `product_category_name`. Không agent nào được bỏ qua field thiếu bằng giá trị suy đoán: trường hợp đó phải ghi vào `missing_or_conflicting_data`.
...
Verifier kiểm tra: output schema/type/enum, giới hạn array, timestamp/null handling, raw entity/evidence IDs, payment and delivery calculations, policy/root cause/responsibility/refund/actions và consistency giữa fields. Errors có prefix `schema`, `limit`, `reference`, `calculation`, `policy` hoặc `consistency` để Coordinator trả đúng owner module.

## Trace và failure handling

Mỗi dòng `logging/trace.jsonl` là JSON event có `run_id`, `case_id`, `sequence`, `event`, `status` và context handoff. Event tối thiểu:

```text
case_started → handoff* → policy_decision → verification_passed|verification_failed
→ output_written (chỉ khi pass) → case_completed
```
...
Script đóng gói chỉ tạo ZIP khi `output/` có chính xác `EC_001.json` đến `EC_050.json`, không có file lạ. ZIP chỉ chứa 50 JSON ở cấp gốc; source, trace, metadata, `.env`, cache và virtual environment không được đưa vào ZIP.
